...
//************************** API CONSTANTS  ***************************
//Topic Status Enum
var cnTopicStatusRelease=0;
var cnTopicStatusBeta=1;
var cnTopicStatusPreliminary=2;

//Header Section Global Variables
var csRelease = "8.0.0.0";
var csCompanyName = "API Healthcare Corporation";
var csHeaderTitle = "Database Documentation";
var cbIsTopicStatusDisplayed = false;
var cnDefaultTopicStatus = cnTopicStatusPreliminary
var isInternal = false;


//Footer Section Global Variables
var cbIsUrlDisplayed = false;
var cnInitialCopyrightYear = 2001;
var cbIsFeedbackLinkDisplayed = false;
var L_TopicFeedbackLink_Text = "Send feedback on this topic";
var L_TopicFeedbackLink_Subject = "Feedback%20on%20Topic";
var L_TopicFeedbackLink_EmailAlias = "Development@api-wi.com" ; //Default Email Address
var L_TopicFeedbackLink_ClassName = "feedback";


//Constants for datetime format
var cnLONGDATE = 1;		//dddd, MMMM dd, yyyy
var cnSHORTDATE = 2;	//MM/DD/YYYY
var cnMMMMYYYY = 3;		//MMMM YYYY
var cnLONGTIME = 1;		//hh:mm:ss 
var cnSHORTTIME = 1;	//hh:mm 
var cnNOFORMAT = 0 ;
...
function StandardHeader(){
	var argv = StandardHeader.arguments;
	var argc = StandardHeader.arguments.length

	//Properties
	this.internalFlag = ((0 < argc) &&(null != argv[0])) ? argv[0]: isInternal;
	this.headerTitle  = ((1 < argc) &&(null != argv[1])) ? argv[1] : csHeaderTitle;
	this.isTopicStatusDisplayed =((2 < argc) && (null != argv[2])) ? argv[2]: cbIsTopicStatusDisplayed;
	this.topicStatus = ((3 < argc) &&(null != argv[3])) ? argv[3]: cnDefaultTopicStatus;
	this.releaseNumber = ((4 < argc) &&(null != argv[4])) ? argv[4]: csRelease;
	this.companyName =((5 < argc) &&(null != argv[5])) ? argv[5]: csCompanyName;
	this.navigationTree = new Array();
	
	
	
	this.topicStatusArray = new Array("[This topic is part of release " + this.releaseNumber + "]",
	...
	//Methods
	this.AddNavigationItem = AddNavigationLink;
	this.Write = StandardHeaderWrite;
	this.toString = StandardHeaderToString;
}
	

function AddNavigationLink(){
	var argv = AddNavigationLink.arguments;
	var argc = AddNavigationLink.arguments.length
	//Properties
	
	this.title  = ((0 < argc) &&(null != argv[0])) ? argv[0] :null;
	this.url =((1 < argc) &&(null != argv[1])) ? argv[1]: null;
	
	this.navigationTree[this.navigationTree.length] = new hyperlink(this.title,this.url);
...
function StandardHeaderToString(){
	
	var sTmp = "";
	sTmp += "<div id='nsbanner'>";
	sTmp += "<div id='bannerrow1'>";
	sTmp += "<TABLE CLASS='bannerparthead' CELLSPACING='0'>";
	sTmp += "<TR id='hdr'>";
	sTmp += "<TD class='runninghead' noWrap>";
	if(this.internalFlag)
		sTmp += "<A HRef = '\\\\racine.api-wi.com\\DevelopmentDocumentation\\DatabaseHelp\\default.html'>";
	sTmp += this.headerTitle;
	if(this.internalFlag)
		sTmp += "</A>";
	sTmp += "</TD>";
	sTmp += "<TD class='product' noWrap>";
	sTmp += this.companyName;
	sTmp += "</TD>";
	sTmp += "</TR>";
	sTmp += "</TABLE>";
	sTmp += "</div>";
	sTmp += "<div id='TitleRow'>";
	sTmp += "<H1 class='dtH1'>"
	for (var link = 0 ; link < this.navigationTree.length; link++){
		if ( (null == this.navigationTree[link].url) && (null == this.navigationTree[link].title) ) {
		}
		else if (null == this.navigationTree[link].url){
			sTmp += "<A NoHRef>" + this.navigationTree[link].title + "</A>";
			if (link<this.navigationTree.length-1){
				sTmp += " : ";
			}
		}
		else{
			sTmp += "<A HRef='" + this.navigationTree[link].url + "'>" +  this.navigationTree[link].title + "</A>";
			if (link<this.navigationTree.length-1){
				sTmp += " : ";
			}
		}
	...
	if (this.isTopicStatusDisplayed) {
		sTmp += "<p class='topicstatus'>";
		sTmp += this.topicStatusArray[this.topicStatus];
		sTmp += "</p>";
	}
	return sTmp;
}//End Function

function StandardHeaderWrite(){
...
function StandardFooter(){
	var argv = StandardFooter.arguments;
	var argc = StandardFooter.arguments.length

	//Properties
	this.isUrlDisplayed  = ((0 < argc) &&(null != argv[0])) ? argv[0] : cbIsUrlDisplayed;
	this.initialCopyrightYear =((1 < argc) && (null != argv[1])) ? argv[1]: cnInitialCopyrightYear;
	this.isFeedbackLinkDisplayed = ((2 < argc) &&(null != argv[2])) ? argv[2] : cbIsFeedbackLinkDisplayed;
	this.emailAlias  = ((3 < argc) &&(null != argv[3])) ? argv[3] : L_TopicFeedbackLink_EmailAlias;
	this.subject =((4 < argc) && (null != argv[4])) ? argv[4]:  L_TopicFeedbackLink_Subject + " - " + document.title;
	this.body = ((5 < argc) && (null != argv[5])) ? argv[5]:  "REF: " + document.location.pathname + " : " + document.title;
	this.text =((6 < argc) && (null != argv[6])) ? argv[6]: L_TopicFeedbackLink_Text;
	this.className = ((7 < argc) && (null != argv[7])) ? argv[7]:  L_TopicFeedbackLink_ClassName;
	
	
	this.onMouseOverColor = "#3366ff";
	this.onMouseOutColor = "#0000FF";
	this.isLineDisplayed = true;
	
	this.navigationTree = new Array();
	
	//methods
	this.toString = StandardFooterToString;
	this.Write =  StandardFooterWrite;
	
}//End Function

function StandardFooterToString() {
		
	var dt = new Date();
	var yr = dt.getFullYear();
	var sTmp = "";
	
	if (this.isFeedbackLinkDisplayed){
		var href = "mailto:"+this.emailAlias+"?subject="+this.subject+"&body="+this.body;
	
		sTmp += "<DIV class="+this.className+">";
		if (this.isLineDisplayed)
			sTmp += "<HR>";
		
			sTmp += "<A class='"+this.className+"' onnmouseout=\"this.style.color = '"+this.onMouseOutColor+"'\"  onmouseover=\"this.style.color = '"+this.onMouseOverColor+"'\" href='" + href +"'>"+this.text+"</A>";	
			sTmp += "<BR><BR>";
			sTmp +="</DIV>";
	}
	sTmp += "<div class='footer'>"
	if (Date.parse(document.lastModified) != 0 ){
		sTmp += "&nbsp;Last Revision: " + FormatDate(document.lastModified,cnLONGDATE,cnLONGTIME)
	}
	
	if (this.isUrlDisplayed)
		sTmp += "<br>&nbsp;URL: " + document.location;
	else
		sTmp += "<br>";

sTmp += "<br>&nbsp;<a class='footer' href='apicopyright.htm' onmouseout=\"this.style.color='#FFFFFF'\" onmouseover=\"this.style.color = '#FF3300'\">&copy; " + this.initialCopyrightYear + " - " + yr + " API Healthcare Corporation. All rights reserved.</a>&nbsp;";	
...
function GetLastModifiedDate(){
	if (Date.parse(document.lastModified) != 0 ){
		return('<P Class=lastmodified>Last Revision: ' + FormatDate(document.lastModified,cnMMMMYYYY,cnNOFORMAT) +  '</P>')
	}
	else {
		return ""
	}
...
function FormatDate(){
	var argv = FormatDate.arguments;
	var argc = FormatDate.arguments.length;
	
	var theDate = (("" != argv[0]) && (argc >0)) ? argv[0] : "";
	var DateFormat = (("" != argv[1]) && (argc >1)) ? argv[1] : cnSHORTDATE;
	var TimeFormat = (("" != argv[2]) && (argc >2)) ? argv[2] : 0;
	...
	var s = theDate.toString();		//Cast the date to a string
	var arrTheDate = s.split(' ');	//Split the date string so we can extract the timezone
	var sTimeZone = arrTheDate[4];	//set the timezone string
	...
	switch (DateFormat){
		case cnNOFORMAT:
			...
			break;
		case cnLONGDATE:
			sDate = Days[theDate.getDay()] + ", " +  Months[theDate.getMonth()] + " " + theDate.getDate() + ", " + theDate.getFullYear();
			break;
		case cnSHORTDATE:
			sDate =  PadNumber(theDate.getMonth() + 1) + "/" + PadNumber(theDate.getDate()) + "/" + theDate.getFullYear();
			break;
		case cnMMMMYYYY:
			 sDate = Months[theDate.getMonth()] + " " + theDate.getFullYear();
			 break;
		default:
			sDate = "Invalid date format."
	}
	...
	switch (TimeFormat){
		case cnNOFORMAT:
			...
			break;
		case cnLONGTIME:
			sTime = PadNumber("at " + theDate.getHours()) + ":" + PadNumber(theDate.getMinutes()) + ":" + PadNumber(theDate.getSeconds()) + " " + sTimeZone;
			break;
		case cnSHORTTIME:
			sTime = PadNumber(theDate.getHours()) + ":" + PadNumber(theDate.getMinutes()) + " " + sTimeZone;
			break;
		default:
...
// Find span associated with bookmark
	...
	for (i=0; i<aColl.length; i++){
		if ( (aColl(i).name != null) && (aColl(i).name != "") ) {
			
			if (aColl(i).name.toUpperCase()==bookmark.toUpperCase()){
				var elem = null
				for(t = 1; t<4; t++){
					if (document.all(aColl(i).sourceIndex + t) != null){
...
function TopicFeedbackLink(){
	var argv = TopicFeedbackLink.arguments;
	var argc = TopicFeedbackLink.arguments.length
	
	//Properties
	this.EmailAlias  = (("" != argv[0]) && (argc >0)) ? argv[0] : L_TopicFeedbackLink_EmailAlias;
	this.Subject = (("" != argv[1]) && (argc >1)) ? argv[1] : L_TopicFeedbackLink_Subject + " - " + document.title;
	this.Body = (("" != argv[2]) && (argc >2)) ? argv[2] : "REF: " + document.location.pathname + " : " + document.title;
	this.Text =(("" != argv[3]) && (argc >3)) ? argv[3] : L_TopicFeedbackLink_Text;
	this.ClassName = (("" != argv[4]) && (argc >4)) ? argv[4] : L_TopicFeedbackLink_ClassName;
	...
	//Methods
	this.Write = WriteTopicFeedbackLink;
}	
function WriteTopicFeedbackLink(){
	//writes feedback link
	href = "mailto:"+this.EmailAlias+"?Subject="+this.Subject+"&body="+this.Body;
	document.writeln("<DIV class="+this.ClassName+">");
	
	if (this.IsLineDisplayed)
		document.writeln("<HR>");
	
	document.writeln("<A class='"+this.ClassName+"' onnmouseout=\"this.style.color = '"+this.OnMouseOutColor+"'\"  onmouseover=\"this.style.color = '"+this.OnMouseOverColor+"'\" href='" + href +"'>"+this.Text+"</A>");	
